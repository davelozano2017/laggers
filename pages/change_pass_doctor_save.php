...
if($_GET['old']){
	...
	if($_SESSION['session_username']== $un){
		...
		$stmt2 = $pdo->prepare("Update user Set PW = :pass, UN = :un Where id = :session_ipid"); 						
		$stmt2->bindParam(':pass', $pass, PDO::PARAM_STR);
		$stmt2->bindParam(':un', $un, PDO::PARAM_STR);
		$stmt2->bindParam(':session_ipid', $_SESSION['session_ipid'], PDO::PARAM_STR);
		...
		if ($res = $pdo->query($sql)) { 
			  ...
			  if ($res->rowCount() > 0) {
				...
				$stmt3 = $pdo->prepare("Update user Set PW = :pass, UN = :un Where id = :session_ipid"); 						
